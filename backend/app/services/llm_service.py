"""LLM Service for interacting with Google Gemini and managing prompts"""
    ...
    def _call_llm(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries and ('quota' in error_msg or 'rate' in error_msg or '429' in error_msg):
                    import time
                ...
                if attempt == max_retries:
                    return self._generate_fallback_response_from_error(prompt)
        
        return self._generate_fallback_response_from_error(prompt)
    
    def _generate_fallback_response_from_error(self, prompt: str) -> str:
        """Generate intelligent fallback when Gemini fails"""
        prompt_lower = prompt.lower()
        
        # Determine what to ask based on context
        if 'email' not in prompt_lower and'@' not in prompt_lower:
            return "Thank you! What's your email address?"
        elif 'phone' not in prompt_lower:
            return "Great! What's your phone number?"
        elif 'experience' not in prompt_lower and 'years' not in prompt_lower:
            return "Perfect! How many years of experience do you have in your field?"
        elif 'position' not in prompt_lower and 'role' not in prompt_lower:
            return "Excellent! What position or role are you interested in?"
        elif 'location' not in prompt_lower and 'city' not in prompt_lower:
            return "Great! Where are you currently located?"
        elif 'tech' not in prompt_lower and 'stack' not in prompt_lower:
            return "Almost done! Please share your tech stack - what technologies, frameworks, and tools do you work with?"
        else:
            return "Thank you for sharing that information! Is there anything else you'd like to add?"
